...
class PrayerTrackerCubit extends Cubit<PrayerTrackerState> {
  ...
  PrayerTrackerCubit({
    required PrayerTrackerService prayerTrackerService,
  ...
  Future<void> initialize() async {
    try {
      ...
      emit(PrayerTrackerError(message: 'Failed to initialize prayer tracker: ${e.toString()}'));
    }
  }
  ...
  Future<void> loadData() async {
    try {
      ...
      final todaysCompletion = await _prayerTrackerService.getTodaysCompletion();
      ...
      emit(PrayerTrackerError(message: 'Failed to load prayer data: ${e.toString()}'));
    }
  }
  ...
  Future<void> togglePrayerCompletion(PrayerType prayer) async {
    ...
    try {
      ...
      final currentCompletions = Map<PrayerType, bool>.from(currentState.todaysCompletion.completions);
      ...
      // Revert on error
      emit(currentState);
      emit(PrayerTrackerError(message: 'Failed to update prayer: ${e.toString()}'));
  ...
  Future<void> resetDaily() async {
    try {
      ...
      emit(PrayerTrackerError(message: 'Failed to reset daily data: ${e.toString()}'));
    }
  }
  ...
  Future<List<PrayerCompletion>> getCompletionsForRange(DateTime start, DateTime end) async {
